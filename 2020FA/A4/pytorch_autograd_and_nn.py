...
def three_layer_convnet(x, params):
  ...
  scores = F.relu(F.conv2d(x, conv_w1, conv_b1, stride=1 ,padding=2))
  scores = F.relu(F.conv2d(scores, conv_w2, conv_b2, stride=1, padding=1))
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  # size = (Dim_out, Dim_in, kernel_size, kernel_size)
  conv_w1 = nn.init.kaiming_normal_(torch.empty(size=(channel_1, C, kernel_size_1, kernel_size_1), dtype=dtype, device=device))
  conv_b1 = nn.init.zeros_(torch.empty(channel_1, dtype=dtype, device=device))
  
  conv_w2 = nn.init.kaiming_normal_(torch.empty(size=(channel_2, channel_1, kernel_size_2, kernel_size_2), dtype=dtype, device=device))
  conv_b2 = nn.init.zeros_(torch.empty(channel_2, dtype=dtype, device=device))
  
  H_out = (H + 2 * 2 - kernel_size_1) // 1 + 1  # After first conv layer
  W_out = (W + 2 * 2 - kernel_size_1) // 1 + 1
  H_out = (H_out + 2 * 1 - kernel_size_2) // 1 + 1  # After second conv layer
  W_out = (W_out + 2 * 1 - kernel_size_2) // 1 + 1
  
  fc_input_dim = channel_2 * H_out * W_out
  
  fc_w = nn.init.kaiming_normal_(torch.empty(size=(num_classes, fc_input_dim), dtype=dtype, device=device))
  fc_b = nn.init.zeros_(torch.empty(num_classes, dtype=dtype, device=device))
  
  # After initialization, need to set requires_grad = True
...
class ThreeLayerConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores = F.relu(self.conv_1(x))
    scores = F.relu(self.conv_2(scores))
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(
    ...
    nn.ReLU(),
    nn.Conv2d(channel_1, channel_2, kernel_size_2, stride=1, padding=1),
    nn.ReLU(),
    Flatten(),
    nn.Linear(32 * 32 * channel_2, num_classes)
  )
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    layers.append(nn.Conv2d(Cin, Cout, kernel_size=3, padding=1, stride=stride))
    layers.append(nn.BatchNorm2d(Cout))
    layers.append(nn.ReLU(inplace=True))
    layers.append(nn.Conv2d(Cout, Cout, kernel_size=3, padding=1, stride=1))
...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    scores = nn.AvgPool2d(kernel_size=(H, W), stride=1)(scores)
    scores = flatten(scores)
