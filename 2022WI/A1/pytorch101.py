...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mu_j = torch.mean(x, dim=0)
    sigma_j = torch.std(x, dim=0)
    y = (x - mu_j) / sigma_j
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    """给定一个包含N个一维张量的列表，每个张量长度不一 (不使用Python循环)
    
    Args:
        xs: 一个包含N个一维张量的列表，每个张量长度可能不同
        ls: 一个长度为N的张量，记录了每个输入张量的长度
    
    Returns:
        y: 长度为N的张量，包含所有平均值
        
    Examples:
        xs = [torch.tensor([1, 2, 3]), torch.tensor([4, 5])]
        ls = torch.tensor([3, 2])
        # 应该返回：tensor([2.0, 4.5])
    """
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # 整体思路：把给定的不等长张量拼接成一个长张量，然后按照累计和作为分割点计算平均值。 
    # 1. 将列表中的张量连接成一个张量, 对于一维张量，cat为直接首尾相连
    x_cat = torch.cat(xs)
    
    # 2. 创建一个累积和数组，用于确定每个原始张量的起始位置(分割点)
    cumsum = torch.cat([torch.tensor([0]), torch.cumsum(ls[:-1], dim=0)])
    
    # 3. 使用 torch.arange 创建索引
    indices = torch.arange(len(ls))
    
    # 4. 计算每个张量的和
    sums = torch.zeros_like(ls, dtype=torch.float)
    sums.index_add_(0, 
                    torch.arange(len(x_cat)), 
                    x_cat, 
                    source_index=torch.searchsorted(cumsum, torch.arange(len(x_cat))))
    
    # 5. 除以长度得到平均值
    y = sums / ls.float()
    
    """with Python loops, (implement by only 1 line)"""
    # y = torch.stack([x.mean() for x in xs])
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    """找出输入张量中K个唯一值，及第一次出现的位置 (使用unique只能得到一半分数)

    Returns:
        uniques: 包含k个唯一值的张量
        indics: 包含这些唯一值第一次出现位置的张量
