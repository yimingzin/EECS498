...
def svm_loss_vectorized(
    ...
    # 除去正确类别影响，如果不除去的话对于正确类别scores - correct_scores = 0，结果为1会影响计算 (正确类别损失为0)
...
def svm_get_search_params():
    ...
    learning_rates = [1e-3, 1e-2, 5e-4, 5e-2, 2e-2]
    regularization_strengths = [1e-2, 1e-2, 1e-1, 1e0, 1e1]
...
def test_one_param_set(
    ...
    train_acc = (y_train == y_train_pred).sum().item() * 100.0 / X_train.shape[0]
    val_acc = (y_val == y_val_pred).sum().item() * 100.0 / X_val.shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        scores_stable = scores - torch.max(scores)
        correct_scores = scores_stable[y[i]]
        sum_of_all_scores = torch.sum(torch.exp(scores_stable))
        # 交叉熵损失 = -log(e^(正确类别得分) / e^(所有类别得分)求和 )
        ...
        for j in range(num_class):
            if j == y[i]:
                dW[:, j] += (torch.exp(scores_stable[j]) / sum_of_all_scores - 1)* X[i]
            else:
                dW[:, j] += (torch.exp(scores_stable[j]) / sum_of_all_scores)* X[i]
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = torch.mm(X, W)
    # 通过keepdim = True 让其保持二维，下softmax同, (max会返回值和索引，只需要值)
