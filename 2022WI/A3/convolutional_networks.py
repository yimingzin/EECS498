...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        N, C, H, W = x.shape
        # 假设有10个卷积核，每个卷积核的形状是(3, 5, 5)，表示每个滤波器跨越3个通道(RGB), 并且滤波器大小为 5x5
        F, C, HH, WW = w.shape
        ...
        out = torch.zeros(size=(N, F, H_prime, W_prime), dtype=x.dtype, device=x.device)
        
        # 图片索引
        for n in range(N):
          # 卷积核索引
          for f in range(F):
            # 当前图片在对应卷积核在图像上滑动，每次移动stride个像素,覆盖一个与卷积核大小相同的感受野,进行卷积计算
            for h_prime in range(H_prime):
              for w_prime in range(W_prime):
                # 感受野大小等于卷积核大小(C, HH, WW)，然后进行卷积操作相乘取和
    ...
    def backward(dout, cache):
          ...
          for f in range(F):
            for h_prime in range(H_prime):
              for w_prime in range(W_prime):
                ...
                # 每次卷积核滑动时，会影响多个位置（每个位置都有对应的感受野）。
                # 反向传播过程中每个输出梯度（dout）会影响多个输入位置的梯度，需要将它们累加
        ...
        # 从第pad个元素开始，到倒数第pad个元素结束：去掉前pad个和后pad个元素
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        out = torch.zeros(size=(N, C, H_prime, W_prime), dtype=x.dtype, device=x.device)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        """
           conv - relu - 2x2 max pool - linear - relu - linear - softmax
           W2根据下面的conv_param, pool_param可计算得到池化后的尺寸变为(num_filters, H//2, W//2)
        """
        C, H, W = input_dims
        self.params["W1"] = torch.normal(mean=0, std=weight_scale, size=(num_filters, C, filter_size, filter_size), dtype=dtype, device=device)
        self.params["b1"] = torch.zeros(num_filters, dtype=dtype, device=device)
        
        self.params["W2"] = torch.normal(mean=0, std=weight_scale, size=(num_filters * (H // 2) * (W // 2), hidden_dim), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        h1, cache_1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        h2, cache_2 = Linear_ReLU.forward(h1, W2, b2)
        scores, cache_fin = Linear.forward(h2, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dh2, grads["W3"], grads["b3"] = Linear.backward(dout, cache_fin)
        grads['W3'] += 2 * self.reg * W3
        dh1, grads["W2"], grads["b2"] = Linear_ReLU.backward(dh2, cache_2)
        grads['W2'] += 2 * self.reg * W2
        
        dx, grads["W1"], grads["b1"] = Conv_ReLU_Pool.backward(dh1, cache_1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(L - 1):
          if i in self.max_pools:
            X, cache_i = Conv_ReLU_Pool.forward(X, self.params[f"W{i+1}"], self.params[f"b{i+1}"], conv_param, pool_param)
            cache.append(cache_i)
          else:
            X, cache_i = Conv_ReLU.forward(X, self.params[f"W{i+1}"], self.params[f"b{i+1}"], conv_param)
            cache.append(cache_i)
        ...
        dh, grads[f"W{L}"], grads[f"b{L}"] = Linear.backward(dout, cache[L-1])
        grads[f"W{L}"] += 2 * self.reg * self.params[f"W{L}"]
        
        for i in range(L-1, 0, -1):
          if i-1 in self.max_pools:
            dh, grads[f"W{i}"], grads[f"b{i}"] = Conv_ReLU_Pool.backward(dh, cache[i-1])
          else:
            dh, grads[f"W{i}"], grads[f"b{i}"] = Conv_ReLU.backward(dh, cache[i-1])
