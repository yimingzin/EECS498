...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channels in stem_channels:
            ...
            nn.init.normal_(conv_rpn.weight.data, 0, 0.01)
        ...
        # Replace "pass" statement with your code
        # 初始化目标性预测层 - 输出通道数为 num_anchors (每个位置的锚框数量)
        self.pred_obj = nn.Conv2d(prev_out, num_anchors, kernel_size=1, stride=1)
        nn.init.normal_(self.pred_obj.weight.data, 0, 0.01)
        nn.init.constant_(self.pred_obj.bias, 0)
        
        # 初始化框回归预测层 - 输出通道数为 4*num_anchors (每个锚框有四个偏移量)
        self.pred_box = nn.Conv2d(prev_out, 4 * num_anchors, kernel_size=1, stride=1)
        nn.init.normal_(self.pred_box.weight.data, 0, 0.01)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        """
            "p3": (batch_size, C, H, W) 对应层级的特征 Tensor
        """
        for key, value in feats_per_fpn_level.items():
            # stem
            stem_rpn_output = self.stem_rpn(value)
            
            object_logits[key] = self.pred_obj(stem_rpn_output) # "p3": batch_size, num_archors, H, W 
            B, N, H, W = object_logits[key].shape
            # 为了确保每个位置的所有锚框的预测值是连续存储的
            object_logits[key] = object_logits[key].permute(0, 2, 3, 1) # "p3": batch_size, H, W, num_archors
            object_logits[key] = object_logits[key].reshape(B, -1)
            
            boxreg_deltas[key] = self.pred_box(stem_rpn_output)         # B, 4N, H, W
            boxreg_deltas[key] = boxreg_deltas[key].permute(0, 2, 3, 1) # B, H, W, 4N
            boxreg_deltas[key] = boxreg_deltas[key].reshape(B, -1, 4)   # B, HWN, 4
...
def generate_fpn_anchors(
    ...
    """
        "p3": (H*W, 2)  # locations is from get_fpn_location_coords()
    """
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            ##################################################################
            # Replace "pass" statement with your code
            width = math.sqrt(area / aspect_ratio) # 特征图越大锚框越大
            height = area / width
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    M = boxes1.shape[0]
    N = boxes2.shape[0]
    iou = torch.zeros(size=(M, N)).to(boxes1)
    ...
    # 计算交集区域坐标
    x1_inner = x1_box2.clamp(min=x1)    # 左上角 x 坐标
    y1_inner = y1_box2.clamp(min=y1)    # 左上角 y 坐标
    x2_inner = x2_box2.clamp(max=x2)    # 右下角 x 坐标
    y2_inner = y2_box2.clamp(max=y2)    # 右下角 y 坐标
    
    H_inner = (y2_inner - y1_inner).clamp(min=0.0) # 若无交集为0
    W_inner = (x2_inner - x1_inner).clamp(min=0.0)
    inner_area = H_inner * W_inner
    
    # 并集面积 = 框1面积 + 框2面积 - 交集面积
    union_area = (area + area_box2) - inner_area
