...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type in ["rnn", "lstm"]:
          self.input = nn.Linear(input_dim * 16, hidden_dim)
        else:
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "rnn":
          h0 = self.input(image_features.reshape(image_features.shape[0], -1))
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "rnn":
          h = self.input(image_features.reshape(image_features.shape[0], -1))
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        hn = torch.zeros(size=(N, T, H), dtype=h0.dtype, device=h0.device)
        prev_h = h0
        prev_c = c0
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        hn = torch.zeros(size=(N, T, H), dtype=A.dtype, device=A.device)
